...
/** Attempts to repair a truncated JSON string by closing open brackets/braces and removing trailing partial key/values. */
...
function sanitizeJSONString(str: string): string {
    const sanitized = str
        ...
        .replace(/[\u0000-\u0008\u000B-\u000C\u000E-\u001F\u007F-\u009F]/g, "") // Keep tabs, newlines, CR
        .trim();
...
export function godSafeParse(raw: string): any {
    ...
    // L4.7: Truncation stack repair
    try {
        const stackRepaired = repairTruncatedJSON(s);
        return JSON.parse(stackRepaired);
    } catch { /* continue */ }
