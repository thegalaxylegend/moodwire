...
export const initAnalytics = () => {
  ...
  if (!(window as any).gtag) {
    console.warn(
      "[Analytics] gtag not found on window object! Verify index.html"
    );
  }
 ...
 */
export const trackQuestionTime = (
  question_id: string,
 ...
 */
export const trackOptionSwitch = (
  question_id: string,
  from_option: string,
  to_option: string
) => {
...
export const trackGlitch = (error: string | Error, component?: string) => {
  ...
  console.error(
    `[Analytics] 🆘 Error caught in ${component || "unknown"}:`,
    errorMsg
  );
};
