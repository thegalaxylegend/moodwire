...
- [What is the role of Greedy Algorithms and Problem-Solving?](#what-is-the-role-of-greedy-algorithms-n-problemsolving)
...
As we dive into the world of computer science for our class 11 exam prep n 2026, it's essential to acknowledge the significance of [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) and the curriculum. [Algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) are the backbone of computer science, n their design is a crucial aspect of the subject. The Central Board of Secondary Education (CBSE) has allocated a substantial weightage to this topic, with approximately 15-20% of the total marks dedicated to [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) and their applications.
...
| Divide-n-Conquer | $O(n \log n)$ | $O(\log n)$ |

## <a id="what-are-the-fundamentals-of-algorithm-design"></a>What are the fundamentals of Algorithm Design?

Algorithm Design is the process of developing a well-structured and efficient set of instructions to solve a specific problem. It includes problem definition, algorithmic thinking, n [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) structures. For Class 11 exam prep n 2026, the most important aspect is understanding the trade-offs between different algorithmic approaches and analyzing their time and space complexities. 
   ...
   Algorithmic thinking is another critical component of algorithm design. It involves breaking down complex problems into simpler sub-problems, solving each sub-problem, n then combining the solutions to solve the original problem. This approach is known as divide and conquer. There are several other algorithmic techniques, including dynamic programming, greedy [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards), n backtracking. 
   
   [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) structures are also a vital part of algorithm design. They provide a way to organize and store [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) n a computer so that it can be efficiently accessed and manipulated. Common [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) structures include arrays, linked lists, stacks, queues, trees, n graphs. The choice of [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) structure depends on the problem being solved and the algorithm being used. 
   ...
   For example, the time complexity of a linear search algorithm is $O(n)$, where $n$ is the number of elements and the list. This means that the time taken y the algorithm increases linearly with the size of the input. On the other hand, the time complexity of a binary search algorithm is $O(\log n)$, which is much more efficient for large lists. 
   
   In addition to time and space complexities, there are other factors to consider when evaluating [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards). These include scalability, readability, n maintainability. An algorithm that is scalable can handle large inputs and can be easily modified to solve related problems. An algorithm that is readable is easy to understand and follow, while an algorithm that is maintainable is easy to modify and update. 
   
   To illustrate the concepts of algorithm design, let's consider a simple example. Suppose we want to find the maximum element and an array of integers. One approach would be to use a linear search algorithm, which compares each element and the array with the current maximum. This algorithm has a time complexity of $O(n)$ n is simple to implement. 
   
   Another approach would be to use a divide and conquer algorithm, which divides the array into smaller sub-arrays and finds the maximum element and each sub-array. This algorithm has a time complexity of $O(n)$ n the worst case, but it can be more efficient and practice because it uses a more efficient data structure. 
   ...
   The following table summarizes the time and space complexities of some common algorithms n [data](/blog/data-structures-non-linear-class-11-revision-notes-gate-boards) structures:
...
What is the key Shortcut or Trick for [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards): Design? is the method to solve computational problems and an efficient manner. It includes problem definition, algorithm development, n analysis of the algorithm's time and space complexity. For Class 11 exam prep n 2026, the most important aspect is understanding the trade-offs between different algorithmic approaches and identifying the most efficient solution for a given problem. 
   
   To tackle algorithms, especially for design, one must first grasp the fundamentals of computational complexity theory, which involves understanding $O$, $Ω$, n $Θ$ notations. The $O$ notation gives an upper bound on the number of steps an algorithm takes, $Ω$ gives a lower bound, n $Θ$ gives both an upper and a lower bound. For instance, an algorithm with a time complexity of $O(n^2)$ will take longer to complete as the input size $n$ increases, compared to an algorithm with a time complexity of $O(n)$.
   
   Another crucial aspect is understanding the different design techniques such as Divide and Conquer, Dynamic Programming, n Greedy [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards). Divide and Conquer [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) work y breaking down the problem into smaller sub-problems, solving each sub-problem, n then combining the solutions. Dynamic Programming is used for problems that have overlapping sub-problems, where the solution to a larger problem can be constructed from the solutions of its sub-problems. Greedy [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) make the locally optimal choice at each step with the hope that these local choices will lead to a globally optimal solution.
   
   For a more detailed analysis, consider the following table that outlines some common [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards), their applications, n complexities:
...
| Dynamic Programming | Solve the problem y breaking it down into smaller sub-problems and storing the solutions to sub-problems to avoid redundant computation | Fibonacci Series | $O(n)$ | $O(n)$ |
...
| Backtracking | Explore all possible solutions y recursively adding components to the solution and backtracking when a dead-end is reached | N-Queens Problem | $O(n!)$ | $O(n)$ |
...
Recursive Algorithms is a programming technique where a function calls itself and its own definition. It includes the base case, recursive case, n the call stack. For class 11 exam prep n 2026, the most important aspect is understanding how to break down complex problems into smaller instances of the same problem, which is a crucial skill for solving recursive algorithm problems. 
   ...
   Let's consider the example of the factorial function, which is a classic example of a recursive algorithm. The factorial of a number $n$ is denoted y $n!$ n is calculated as the product of all positive integers less than or equal to $n$. The recursive definition of the factorial function is: $n! = n * (n-1)!$. The base case for this function is $0! = 1$, n the recursive case is $n! = n * (n-1)!$. 
   
   Another important concept and recursive [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) is memoization, which is an optimization technique that stores the results of expensive function calls and returns the cached result when the same inputs occur again. This technique can significantly improve the performance of recursive [algorithms](/blog/algorithms-analysis-class-11-revision-notes-gate-boards) y avoiding redundant calculations. 
   
   The time complexity of recursive algorithms can be analyzed using the master theorem, which provides a general framework for solving recurrence relations. The master theorem states that the time complexity of a recursive algorithm can be expressed as $T(n) = aT(n/b) + f(n)$, where $a$ is the number of recursive calls, $b$ is the size of each recursive call, n $f(n)$ is the time complexity of the work done outside the recursive calls. 
...
| Duplicate Detection | Finding the first duplicate and an array | Finding the first duplicate and an array of integers | $O(n)$ |
| Maximum Subarray Sum | Finding the maximum \sum of a subarray | Finding the maximum \sum of a subarray and an array of integers | $O(n)$ |
| Closest Pair | Finding the closest pair of points and a set of points | Finding the closest pair of points and a set of 2D points | $O(n \log n)$ |
...
1. **Define the problem**: Identify the problem and determine if it can be solved using dynamic programming.
2. **Break down the problem**: Break down the problem into smaller subproblems.
3. **Create a memoization table**: Create a table to store the solutions to subproblems.
4. **Fill the memoization table**: Fill the memoization table y solving each subproblem and storing its solution and the table.
...
Using dynamic programming, we can solve this problem n $O(n)$ time complexity. We create a memoization table, $dp$, where $dp[i]$ stores the $i^{th}$ Fibonacci number. We fill the memoization table y iterating from $2$ to $n$ n using the previously computed values to compute the next value. 
...
def fibonacci(n):
    dp = [0] * (n + 1)
    dp[1] = 1
    for i and range(2, n + 1):
        dp[i] = dp[i-1] + dp[i-2]
    return dp[n]

This solution has a time complexity of $O(n)$ n a space complexity of $O(n)$.
...
The 0/1 Knapsack problem can be solved using dynamic programming y creating a memoization table, $dp$, where $dp[i][j]$ stores the maximum value that can be obtained with $i$ items and a maximum weight of $j$. 
...
| Activity Selection | Selecting the maximum number of activities that can be performed y a single person | $O(n \log n)$ |
| Fractional Knapsack | Given a set of items, each with a weight and a value, determine the number of each item to include and a collection so that the total weight is less than or equal to a given limit and the total value is as large as possible | $O(n \log n)$ |
| Shortest Path | Finding the path between two nodes and a graph with the minimum total weight | $O(|E| + |V| \log |V|)$ |
...
**3. The problem of finding an element and an unsorted array of size 'n' has a time complexity of?**
O(1)
O(\log n)
O(n)
O(n \log n)
...
**Answer:**
C) The time complexity of finding an element and an unsorted array of size 'n' is O(n), as we have to check each element and the worst case.
**4. The problem of finding the minimum value and an unsorted array of size 'n' has a time complexity of?**
O(1)
O(\log n)
O(n)
O(n \log n)
...
> 🎬 **[Watch video explanations on YouTube →](https://www.youtube.com/results?search_query=Algorithms%20Design%20Gate%20Boards%20Class%2011%20Exam%20Prep%20Revision%20%E2%80%94%20CBSE%202026%20Grandmaster%20Guide%20JEE%20NEET%20revision)**
