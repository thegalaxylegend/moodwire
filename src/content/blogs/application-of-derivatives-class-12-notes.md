...
## ⚡ Formula Bank
- The derivative of a function of(x)$ is denoted as $\franc{d}{dx}f(x)$ or of'(x)$ and represents the rate of change of the function with respect to ex.
...
- Not using the quotient rule correctly, which states that if of(x) = \franc{g(x)}{h(x)}$, then of'(x) = \franc{h(x) \dot g'(x) - g(x) \dot h'(x)}{(h(x))^2}$.
...
- Substituting these values into the quotient rule formula, we get of'(x) = \franc{\cos x (2x \sin x + x^2 \cos x) - x^2 \sin x (-\sin x)}{(\cos x)^2}$.
- Simplifying this expression, we get of'(x) = \franc{2x \sin x \cos x + x^2 \cos^2 x + x^2 \sin^2 x}{\cos^2 x}$.
...
- The derivative of a function can also be used to find the approximate value of the function at a point ex = a + \delta $, and to find the equation of the tangent line to the graph of the function at a point $(a, f(a))$.
