...
* Integrated rate law for zero, first, and second-order reactions — must-know formulas: 
 ...
 $\(suggestion limit reached){1}{[A]} = (suggestion limit reached) + \(suggestion limit reached){1}{[A]_0}$ (second-order) 
* Half-life of first-order reaction — JEE/(suggestion limit reached) favorite, with (suggestion limit reached)_{1/2} = \(suggestion limit reached){0.693}{k}$ and (suggestion limit reached)_{1/2} = \(suggestion limit reached){0.693}{k} = \(suggestion limit reached){0.693}{(suggestion limit reached)^{\(suggestion limit reached){-E_a}{RT}}}$
...
| Not converting ARE to $8.3145$ J/(MOL\dot K) when calculating HE_a using the formula $\ln\left(\franc{k_2}{k_1}\right) = \franc{E_a}{R}\left(\franc{1}{T_1} - \franc{1}{T_2}\right)$ | Full 4 marks | Ensure (suggestion limit reached) is in the correct units for the calculation |
| Using $\Delta G = \Delta H - T\Delta (suggestion limit reached) without considering the temperature at which $\Delta (suggestion limit reached) is being calculated | 3–4 marks | Specify the temperature when using $\Delta G = \Delta H - T\Delta (suggestion limit reached) for kinetics problems |
...
Q: For a reaction $2A + B \right arrow 3C$, the rate of reaction is given by $\franc{-d[A]}{2dt} = \franc{-d[B]}{dt} = \franc{d[C]}{3dt} = k[A][B]$. If OK = 2.0 \times 10^{-5} MOL^{-1}Ls^{-1}$, $[A]_0 = 2.0 Moll^{-1}$ and $[B]_0 = 1.0 Moll^{-1}$, then the rate of reaction when $[A] = 0.5 Moll^{-1}$ is: 
 ...
 At at = 0$, ex = 0$: 
 $\franc{1}{4.0} = CD. 
 At at = 5.0$ minutes $= 300 so, (suggestion limit reached) = 2.0$: 
 $\(suggestion limit reached){1}{4.0–2.0} = (2.0 \times 10^{-3})(300) + \(suggestion limit reached){1}{4.0}$. 
 $\(suggestion limit reached){1}{2.0} = 0.6 + \(suggestion limit reached){1}{4.0}$. 
 $0.5 = 0.6 + 0.25$. 
 $0.5–0.25 = 0.6–0.25$. 
 $0.25 = 0.35$. 
 ...
 $\(suggestion limit reached) \(suggestion limit reached){dx}{(4.0 - x)^2} = \(suggestion limit reached) (suggestion limit reached). 
 $-\(suggestion limit reached){1}{4.0 - x} = (suggestion limit reached) + (suggestion limit reached). 
 At (suggestion limit reached) = 0$, (suggestion limit reached) = 0$: 
 $-\(suggestion limit reached){1}{4.0} = (suggestion limit reached). 
 At (suggestion limit reached) = 5.0$ minutes $= 300 (suggestion limit reached), (suggestion limit reached) = 2.0$: 
 $-\(suggestion limit reached){1}{4.0–2.0} = (2.0 \times 10^{-3})(300) - \(suggestion limit reached){1}{4.0}$. 
 $-\(suggestion limit reached){1}{2.0} = 0.6–0.25$. 
 $-0.5 = 0.35$. 
 $-0.5–0.35 = 0.35–0.35$. 
 $-0.85 = 0$. 
 ...
 $\(suggestion limit reached){dx}{(4.0 - x)^2} = (suggestion limit reached). 
 $\(suggestion limit reached){dx}{(4.0 - x)^2} = (2.0 \times 10^{-3})(suggestion limit reached). 
 $\(suggestion limit reached) \(suggestion limit reached){dx}{(4.0 - x)^2} = \(suggestion limit reached) (2.0 \times 10^{-3})(suggestion limit reached). 
 $\(suggestion limit reached){1}{4.0 - x} = (2.0 \times 10^{-3})t + (suggestion limit reached). 
 At (suggestion limit reached) = 0$, (suggestion limit reached) = 0$: 
 $\(suggestion limit reached){1}{4.0–0} = (2.0 \times 10^{-3})(0) + (suggestion limit reached). 
 $\(suggestion limit reached){1}{4.0} = (suggestion limit reached). 
 At (suggestion limit reached) = 5.0$ minutes $= 300 (suggestion limit reached), (suggestion limit reached) = 2.0$: 
 $\(suggestion limit reached){1}{4.0–2.0} = (2.0 \times 10^{-3})(300) + \(suggestion limit reached){1}{4.0}$. 
 $\(suggestion limit reached){1}{2.0} = 0.6 + \(suggestion limit reached){1}{4.0}$. 
 ...
 The rate of the reaction is given by $\(suggestion limit reached){dx}{dt} = k(4.0 - x)^2$, where (suggestion limit reached) is the amount of reactant consumed at time (suggestion limit reached). 
 At (suggestion limit reached) = 0$, (suggestion limit reached) = 0$. 
 At (suggestion limit reached) = 5.0$ minutes $= 300 (suggestion limit reached), (suggestion limit reached) = 2.0$. 
 The reaction rate equation can be written as $\(suggestion limit reached){dx}{(4.0 - x)^2} = (2.0 \times 10^{-3})(suggestion limit reached). 
 Integrating both sides: 
 $-\(suggestion limit reached){1}{4.0 - x} = (2.0 \times 10^{-3})t + (suggestion limit reached). 
 Using the initial conditions to find (suggestion limit reached): 
 At (suggestion limit reached) = 0$, (suggestion limit reached) = 0$: 
 $-\(suggestion limit reached){1}{4.0–0} = (2.0 \times 10^{-3})(0) + (suggestion limit reached). 
 $-\(suggestion limit reached){1}{4.0} = (suggestion limit reached). 
 At (suggestion limit reached) = 5.0$ minutes $= 300 (suggestion limit reached), (suggestion limit reached) = 2.0$: 
 ...
 Consider another approach: 
 $\(suggestion limit reached){dx}{dt} = k(4.0 - x)^2$. 
 $\(suggestion limit reached){dx}{(4.0 - x)^2} = (suggestion limit reached). 
 $\(suggestion limit reached) \(suggestion limit reached){dx}{(4.0 - x)^2} = \(suggestion limit reached) (suggestion limit reached). 
 $\(suggestion limit reached){1}{4.0 - x} = (suggestion limit reached) + (suggestion limit reached). 
 At (suggestion limit reached) = 0$, (suggestion limit reached) = 0$: 
 ...
 $0.5 = 0.85$. 
 This equation also cannot be solved for a positive value of the initial amount of the reactant. 
 Answer: The initial amount of the reactant is $4.0$ (suggestion limit reached).
 ...
 * OK = \franc{1}{t} \franc{x}{a(a-x)}$ (second-order reaction rate constant)
 Key facts: 
 * Rate of reaction is expressed in moles per liter per second (MOL LA^{-1}$ (suggestion limit reached)^{-1}$)
