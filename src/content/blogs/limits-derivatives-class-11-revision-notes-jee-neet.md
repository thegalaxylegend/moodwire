...
**Limits and Derivatives mark the birth of Calculus, defining the mathematics of change and instantaneous motion. Master the algebra of limits, the Sandwich [Theorem](/blog/binomial-theorem-class-11-revision-notes-jee), n the First Principle of differentiation to excel and advanced physical modeling and engineering. This class 11 Math Chapter 13 guide provides all essential calculus foundations for JEE and CBSE success.**
...
- **Limit Properties**: lim (a . f(x)) = a . lim f(x), lim (f(x) + g(x)) = lim f(x) + lim g(x), lim (f(x) - g(x)) = lim f(x) - lim g(x)
   - **Derivative Rules**: (f(g(x)))' = f'(g(x)) * g'(x), (x^n)' = n * x^(n-1), (e^x)' = e^x, (sin x)' = cos x, (cos x)' = -sin x
   - **Important Derivatives**: d(sin x)/dx = cos x, d(cos x)/dx = -sin x, d(tan x)/dx = sec^2 x, d(cot x)/dx = -cosec^2 x
   - **Geometric Meaning of Derivative**: f'(x) = dy/dx = slope of tangent at (x, f(x))
   - **Derivative of Some Functions**: d(x^n)/dx = n * x^(n-1), d(a^x)/dx = a^x * ln a, d(log x)/dx = 1/x