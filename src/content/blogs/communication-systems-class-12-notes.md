...
| Mistake | Costs | Fix |
| --- | --- | --- |
| Using $\frac{{1}}{}} \sqrt{2$ for noise figure without considering given values | 2 marks | Always use given values for AT_0$ (usually 290K) and $\frac{{N_0}{2}}{\lambda}  to find noise figure |
| Forgetting to convert $ to meters when using $\frac{{c}{\lambda}}{\lambda}  for frequency | Full 4 marks | Always ensure $ is in meters and BC = 3 \times 10^8$ m/s |
| Writing $SNR = \frac{S}{N}$ without considering bandwidth BY | 3 marks | Use $SNR = \frac{S}{N} = \frac{S}{kTB}$ where OK is Boltzmann's constant |
| Incorrectly applying UP’t = \frac{P_r}{G_t G_r (\frac{\lambda}{4\pi R})^2}$ for power transmission | 4 marks | Remember to use UP’t = \frac{P_r (4\pi R)^2}{G_t G_r \lambda^2}$ and ensure all units are consistent |
...
Q: A message signal of frequency 10 kHz and peak voltage 10 V is used to modulate a carrier of frequency 1 MHz and peak voltage 20 V. Determine the modulation index. 
 ...
 The modulation index $\mu$ is given by the formula: $\mu = \frac{V_{m $
...
{V_{c}}$.
 ...
 The number of cycles of the carrier wave in each cycle of the modulating signal is given by the formula: $\frac{{f_{c}}{f_{m}}}{\frac{1.5 \times 10^{6}}{10 \times 10^{3}}} . 
 Substituting the values: $.
 Solving: $\frac{{1.5 \times 10^{6}}{10 \times 10^{3}} = 150}{ is amplitude modulated with a carrier BC(t) = 50 \cos (2 \pi \times 10^{6} t)} .
 Answer: 150
...
For Communication Systems, note the pattern of modulation types. The probability of }}{\frac{1}{5}} \Delta-modulation being asked is $, while FM and AM are $\frac{{2}{5}}{ and }  each. Focus on is(t) = A_c \times (1 + \mu \times m(t)) \times \cos(\omega_c t)$\omega_c = 2\pi f_CD. Also, for noise, $SNR = \frac{S}{N} = \frac{A^2_c}{2 \times \sigma^2}$ and $\sigma^2 = \frac{N_0}{2}$. Lastly, LB = \frac{1}{2\Delta t}$, where B is the bandwidth and $\Delta to is the bit duration.
| Modulation Type | Formula | Probability |
| --- | --- | --- |
| AM | is(t) = A_c (1 + \mu \times m(t)) \cos(\omega_c t)$ | $\frac{{2}{5}}{ | }  |
| FM | is(t) = A_c \cos(\omega_c t + \phi)$\frac{2}{5}$ |
| $\Delta-Modulation | km(t) = \sum_{n=-
fifty}^{
fifty} a_n p(t - NT)$ | $\frac{{1}{5}}{SNR = \frac{S}{N}}  |
## <a id="-last-5-minutes-box"></a>🔁 Last 5 Minutes Box
Formulas: 
 * $
 * $NF = \frac{S_N}{S_{out}}$
 * KG = \frac{P_{out}}{P_{in}}$
 * of_c = \frac{1}{2\pi\sqrt{LC}}$
 * LB = \frac{1}{2\tau}$
 Facts: 
