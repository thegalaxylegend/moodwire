...
- $E = \frac{-0.0592}{n} \log \frac{[Red]}{[Ox]}$ for calculating the reduction potential.

 - $\Delta G = -nFE$ for relating Gibbs free energy to electromotive force.

 - $K_{eq} = \frac{[P]}{[R]}$ for equilibrium constant of a reaction.

 - $pH = -\log[H^+)$ for calculating pH from hydrogen ion concentration.
 ...
 - Common mistake: Forgetting that $n$ in $\Delta G = -nFE$ represents the number of electrons transferred.
