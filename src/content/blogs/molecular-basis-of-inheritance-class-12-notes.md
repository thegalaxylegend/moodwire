...
- **Mistake 1:** Incorrectly identifying the $\times$ symbol in genetic problems as a multiplication sign instead of a cross for hybridization.

 - *Costs:* 2-3 marks for each incorrect interpretation.

 - *Fix:* Always recognize the $\times$ symbol in genetic problems as indicating a cross between two parents.

 - **Mistake 2:** Forgetting to apply the $2^n$ rule for determining the number of possible genotypes in a monohybrid or dihybrid cross, where $n$ is the number of genes.

 - *Costs:* 4-5 marks for each incorrect calculation.
 ...
 - *Fix:* Recognize that linked genes are located on the same chromosome and tend to be inherited together, while unlinked genes are located on different chromosomes and have independent assortment.

 - **Mistake 4:** Incorrectly calculating the probability of certain genotypes or phenotypes using the laws of probability, particularly the product rule $P(A \cap B) = P(A) \times P(B)$.
...
- **Q1:** What is the distance between two consecutive nucleosomes in a eukaryotic chromosome?

 - **Trap:** Confusing the structure of nucleosomes and the packing of DNA
 ...
 - **Trap:** Not understanding the role of the lac repressor and the inducer
 ...
 - **Answer:** The lac genes are not transcribed

 - **Q3:** A couple has a child with cystic fibrosis, an autosomal recessive disorder. If both parents are heterozygous for the cystic fibrosis gene, what is the probability that their next child will have cystic fibrosis?

 - **Trap:** Not applying the Punnett square correctly

 - **Solution:** 

 Let $C$ be the dominant allele (normal gene) and $c$ be the recessive allele (cystic fibrosis gene). Both parents are heterozygous, $Cc$. 
...
| --- | --- | --- |
| Lac repressor protein | Binds to operator region, preventing transcription | Repressed by allolactose |
...
- $p \	imes q$ represents the probability of getting the recessive allele in a monohybrid cross.
 ...
 - The Hardy-Weinberg principle describes the genetic equilibrium in a population.

 - DNA replication is semi-conservative, as demonstrated by the Meselson-Stahl experiment.

 - Common mistakes:

 - Forgetting that $p$ represents the frequency of the dominant allele and $q$ represents the frequency of the recessive allele.
