...

{
 ...
 "body": "- **Q1:** Simplify: $({(\frac{1}{3})}^{-2} - {(\frac{1}{2})}^{-3}) \div {(\frac{1}{4})}^{-2}$
...
- **The Hidden Pattern:** When dealing with exponents and powers in exam questions, a pattern emerges after analyzing 5+ years of PYQs. This pattern involves the frequent use of the formula $a^m 	imes a^n = a^{m+n}$ and its variations, such as $a^m div a^n = a^{m-n}$ and $(a^m)^n = a^{mn}$.

- **For instance, the formula can be applied as follows: a^m 	imes a^n = a^{m+n}, 
 "
...
- Standard form of a number is $k \times 10^n$, where $1 \le k < 10$ and $n$ is an integer.
...
- Do NOT confuse $a^m + a^n$ with $a^{m+n}$. Exponents only add when multiplying powers with the same base.
