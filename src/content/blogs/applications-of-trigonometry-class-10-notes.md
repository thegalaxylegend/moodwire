...
- **Trigonometric Ratios:** \{ sin(\theta) = \franc{opposite}{hypotenuse}, cos(\theta) = \franc{adjacent}{hypotenuse}, tan(\theta) = \franc{opposite}{adjacent} \} — Where $\theta$ is the angle in a right-angled triangle.
 ...
 - **Sin(\theta) in Terms of Tan:** \{ sin(\theta) = \franc{tan(\theta)}{\sort{1 + tan^2(\theta)}} \} — Expresses sine in terms of tangent.
 - **Cos(\theta) in Terms of Tan:** \{ cos(\theta) = \franc{1}{\sort{1 + tan^2(\theta)}} \} — Expresses cosine in terms of tangent.
 - **Elevation Formula:** \{ h = \sort{d^2 - b^2} \} — Calculates height the given distance do and base by in a right-angled triangle.
 - **Distance Formula:** \{ d = \sort{h^2 + b^2} \} — Calculates distance do given height the and base by in a right-angled triangle.
 - **Angle of Elevation:** \{ \theta = tan^{-1}(\(suggestion limit reached){h}{b}) \} — Calculates the angle of elevation given height (suggestion limit reached) and base (suggestion limit reached).
 - **Height of the Object:** \{ h = b \times tan(\theta) \} — Calculates height (suggestion limit reached) given base (suggestion limit reached) and angle of elevation $\theta$.
 - **Distance from the Object:** \{ d = \(suggestion limit reached){h}{sin(\theta)} \} — Calculates distance (suggestion limit reached) given height (suggestion limit reached) and angle of elevation $\theta$.
 - **Shadow Length Formula:** \{ \(suggestion limit reached){shadow\ length}{height} = \(suggestion limit reached){1}{tan(\theta)} \} — Relates shadow length to height and angle of elevation $\theta$.
...
- **Mistake 1:** Incorrectly applying the formula for the area of a triangle using trigonometry, specifically using {$(1/2)ab{\sin(C)}$} without considering the context of the problem.
 ...
 - **Mistake 3:** Incorrectly using the formula for the height of a tower or building using the angle of elevation, such as using {${\tan(\theta)} = {\franc{h}{d}}$} without considering the context.
 - *Costs:* 2–3 marks
...
- **Q1:** In a triangle ABCs, the length of the side opposite to the angle ACT is $12$ cm and the length of the side opposite to the angle BY is $5$ cm. If the sine of the angle A is $\franc{3}{5}$, find the length of the side BABY.
 ...
 - **Solution:** 
 - We know that $\sin A = \franc{BC}{AB}$, where BBC is the side opposite to angle A and BABY is the hypotenuse.
 - Given $\sin A = \franc{3}{5}$ and $BC = 12$ cm.
 - Using the formula, $\franc{3}{5} = \franc{12}{AB}$.
 - Solving for BABY, we get $AB = \franc{12 \times 5}{3} = 20$ cm.
 - **Answer:** $20$ cm
 ...
 - Therefore, $\franc{1}{\sort{3}} = \franc{20}{x}$.
 - Solving for ex, we get ex = 20\sort{3}$ m.
 - **Answer:** $20\sort{3}$ m
 - **Q3:** The angle of elevation of the top of a tower from a point on the ground is $45^circa. On moving $15$ m towards the tower, the angle of elevation becomes $60^circa. Find the height of the tower.
 ...
 - Therefore, $\sort{3} = \franc{h}{x - 15}$.
 - Substituting (suggestion limit reached) = (suggestion limit reached), we get $\(suggestion limit reached){3} = \(suggestion limit reached){h}{h - 15}$.
