...
export const isVideoMatchForExam = (title: string, channelName: string, exam: string): boolean => {
    ...
    if (examLower.includes('jee')) {
        ...
        if (bioKeywords.some(keyword => titleLower.includes(keyword) || channelLower.includes(keyword))) {
            return false;
        }
    }
    if (examLower.includes('neet')) {
        // Enforce: No mathematics
        const mathKeywords = ['maths', 'mathematics', 'math', 'jee math', 'jee maths', 'jee-math', 'jee main math'];
        if (mathKeywords.some(keyword => titleLower.includes(keyword) || channelLower.includes(keyword))) {
            return false;
        }
    }
    return true;
...
export const getVideoByTopicId = async (topicId: string, exam: string = 'JEE', studentClass: string = ''): Promise<Playlist | null> => {
    ...
    const searchQuery = (topicId.split(' ').length > 3)
        ? topicId
        : buildSearchQuery(topicId, exam, studentClass);
    ...
    try {
        ...
        for (const item of searchData.items) {
            ...
            // Strict Filter: Don't show biology for JEE and math for NEET
            if (!isVideoMatchForExam(item.snippet.title, item.snippet.channelTitle, exam)) continue;
...
export const getStrategicVideoRecommendations = async (
    ...
    try {
        ...
        if (recommendedVideos.length < 4) {
            const isMedical = exam.toLowerCase().includes('neet') || exam.toLowerCase().includes('medical');
            const generalTopics = isMedical ? ['physics', 'chemistry', 'biology'] : ['physics', 'chemistry', 'math'];
