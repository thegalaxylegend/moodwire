...
import { predictionService, getExamConstants } from './predictionService';
import {
    doc,
    getDoc,
    collection,
...
export const calculatePredictedRank = (percentage: number, examType: string = 'JEE Mains') => {
    const maxScore = getExamConstants(examType).MAX_SCORE;
    const score = (percentage / 100) * maxScore;

    // Use the unified prediction service to ensure identical mapping across the app
    const { rank } = predictionService.scoreToRank(score, examType);
