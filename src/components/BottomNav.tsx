    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden pointer-events-none">
            ...
            <div className="pointer-events-auto px-4 pb-4 pt-2">
                <nav className="glass-card flex items-center justify-between px-2 py-2 shadow-2xl bg-surface/90 backdrop-blur-xl border-t border-white/5">
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`p-1.5 rounded-xl transition-all duration-300 ${isActive ? 'bg-primary/10 translate-y-[-4px]' : 'group-active:scale-90'}`}>
                                    <item.icon size={22} className={`oxygen-icon ${isActive ? 'fill-primary/20' : ''}`} />
                                </div>
